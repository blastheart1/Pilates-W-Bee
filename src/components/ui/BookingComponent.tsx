...
const CustomCalendar = ({ selectedDate, onSelect, disabled }) => {
  ...
  const generateCalendarDays = () => {
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      days.push({ day, date, isDisabled, isSelected, isToday });
    }
    return days;
  };
  ...
  const handleDateClick = (dateObj) => {
    if (!dateObj.isDisabled && onSelect) onSelect(dateObj.date);
  };
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg w-full max-w-md sm:max-w-lg lg:max-w-4xl max-h-[calc(100vh - env(safe-area-inset-top) - env(safe-area-inset-bottom))] overflow-y-auto relative">
        ...
        <div className="p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8">
          {/* Left Column: Calendar & Recurring */}
          ...
          {/* Right Column: Time Slots & Confirm */}
          <div className="space-y-6">
            <div className="bg-gray-50 p-4 sm:p-6 rounded-lg">
              <h3 className="text-xl font-medium mb-4 flex items-center">
                <Clock className="mr-2 text-pink-600" size={20} />Available Time Slots
              </h3>
              {selectedDate ? availableSlots.map((slot, i) => (
                <div key={i} className={`p-4 rounded-lg border-2 cursor-pointer transition-all ${slot.available ? "border-gray-200 hover:border-pink-600 hover:bg-pink-50" : "border-orange-200 bg-orange-50"}`}>
                    ...
                    <div className="text-right">
                      {slot.available ? (
                        ...
                        <div className="text-center">
                          ...
                          <button className="text-xs text-blue-600 hover:underline mt-1">Join Waitlist ({slot.waitlist})</button>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <button 
              className={`w-full bg-pink-600 hover:bg-pink-700 text-white text-lg py-4 rounded-lg font-medium transition-colors ${!selectedDate ? 'opacity-50 cursor-not-allowed' : ''}`} 
              disabled={!selectedDate} 
              onClick={() => { alert("Class booked successfully!"); onClose(); }}
            >
              Confirm Booking
            </button>
          </div>
